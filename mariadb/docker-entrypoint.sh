...
docker_temp_server_start() {
	mysqld --skip-networking --socket="${SOCKET}" --wsrep_on=OFF &
...
docker_init_database_dir() {
	...
	installArgs=( --datadir="$DATADIR" --rpm --wsrep_on=OFF)
...
_main() {
	...
	if [ "$1" = 'mysqld' ] && ! _mysql_want_help "$@"; then
		...
		mysql_note "Checking MySQL configuration variables.."
		mysql_check_config "$@"
		# Load various environment variables
		mysql_note "Checking the Docker setup environment.."
		docker_setup_env "$@"
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			...
			mysql_note "Initializing database directory.."
			docker_init_database_dir "$@"
			...
			mysql_note "Setting up database.. "
			docker_setup_db
